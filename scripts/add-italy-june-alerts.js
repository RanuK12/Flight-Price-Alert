#!/usr/bin/env node
/**
 * Crea alertas one-way Argentina → Italia (cualquier aeropuerto italiano),
 * fechas 7-10 jun 2026, threshold € 500. Idempotente: usa el upsert de
 * routesRepo.createRoute, por lo que correrlo N veces no duplica.
 *
 * Aeropuertos italianos considerados (todos los intl de pasajeros con
 * vuelos intercontinentales / conexiones frecuentes desde Argentina):
 *   FCO Roma-Fiumicino
 *   MXP Milán-Malpensa
 *   LIN Milán-Linate
 *   BGY Milán-Bérgamo (Ryanair hub)
 *   VCE Venecia
 *   BLQ Bolonia
 *   NAP Nápoles
 *   CTA Catania
...
const DESTS = flag(
  '--dests',
  'FCO,MXP,LIN,BGY,VCE,BLQ,NAP,CTA,PMO,FLR,TRN',
).split(',').map((s) => s.trim().toUpperCase()).filter(Boolean);
...
async function main() {
  ...
  console.log(' Nueva alerta: Argentina → Italia, jun 7-10, ≤ €' + THRESHOLD);
  console.log('━'.repeat(60));
